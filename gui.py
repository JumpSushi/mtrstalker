...
class mtrstalker:
    ...
    def _setup_map(self):
        """Initialize the map with station positions"""
        stations = list(self.api.STATION_NAMES.keys())
        
        # Draw the main line with green color
    ...
    def update_display(self, trains):
        """Update the GUI with new train information."""
        if trains is None:
            return
    ...
    def _continuous_map_update(self, up_trains, down_trains):
        ...
        # Schedule next update if window is still running
        if self.running:
            self.root.after(self.map_update_interval, 
                          lambda: self._continuous_map_update(up_trains, down_trains))
    ...
    def _show_train_info(self, event):
        """Display train information on click"""
        x, y = event.x, event.y
    ...
    def update_loop(self):
        """Background loop to update train information."""
        while self.running:
            try:
